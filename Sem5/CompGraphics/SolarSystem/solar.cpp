...
class Planet {
    ...
    void setdata(string name, double a, double b, double speed, double theta =0,int radius=100, int color=WHITE){
        this->name = name;
        this->a = a;
        this->b = b;
        this->speed = speed*pi/180.0;
...
class SolarSystem{
    ...
    SolarSystem(){
        for(int i=0;i<MAX_PLANETS;i++){
            this->planets[i] = new Planet(planet_names[i], planet_a[i], 0.6*planet_a[i], planet_speed[i], planet_theta[i], planet_radius[i]/11, planet_color[i]);
        }
    }
